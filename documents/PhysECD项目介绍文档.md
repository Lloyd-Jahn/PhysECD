...
4. **多任务学习**：同时监督激发能（E）、速度电偶极矩（μ_vel）和旋转强度（R）；磁偶极矩（m）通过 R 的点积损失隐式约束（当前 `lambda_m=0.0`，不作直接监督）
...
**多任务损失（EMA 动态归一化）**：
```
L_total = λ_E · (L_E / EMA_E) + λ_μ_vel · (L_μ_vel / EMA_μ_vel) + λ_m · (L_m / EMA_m) + λ_R · (L_R / EMA_R)
```

各分量通过指数移动平均（EMA，decay=0.99）追踪其量级并动态归一化，使各任务损失始终在同一数量级上，无需手动调整量级比例。
...
2. **速度电偶极矩损失（相位不变损失）**：
   ```
   # 量子波函数存在全局相位符号不定性，因此取 "同号" 和 "反号" 两种情形的最小值
   L_μ_vel = mean( min(‖μ_vel,pred - μ_vel,true‖², ‖μ_vel,pred + μ_vel,true‖²) )
   ```

3. **磁偶极矩损失（相位不变损失，当前权重为 0）**：
   ```
   L_m = mean( min(‖m_pred - m_true‖², ‖m_pred + m_true‖²) )
   ```
   > 当前 `λ_m = 0.0`，磁偶极矩损失暂不参与训练，因为旋转强度 R 本身已隐式包含了 μ_vel·m 的约束。

4. **旋转强度损失（L1 Loss）**：
   ```
   L_R = L1(R_pred, R_true)
   ```
   R_pred 与 R_true 均使用 10^-40 cgs 单位（见第 3.3 节）。使用 L1 而非 MSE 以提高对极端值的鲁棒性。

**辅助指标**：
- `R_sign_acc`：旋转强度符号预测准确率（仅用于监控，不参与反向传播）

**权重设置**：
```python
lambda_E = 1.0       # 激发能
lambda_mu_vel = 1.0  # 速度电偶极矩
lambda_m = 0.0   # 磁偶极矩（当前关闭）
lambda_R = 1.0   # 旋转强度
```
