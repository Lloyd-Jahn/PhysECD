...
def plot_loss_curves(train_losses, val_losses, save_path):
    ...
    plt.subplot(3, 3, 3)
    plt.plot(steps, train_losses['mu'], 'b-', label='Train', linewidth=2)
    plt.plot(steps, val_losses['mu'], 'r-', label='Validation', linewidth=2)
    plt.xlabel('Step')
    plt.ylabel('Normalized Loss')
    ...
    plt.subplot(3, 3, 7)
    plt.plot(steps, train_losses['mu_raw'], 'b-', label='Train (Raw)', linewidth=2)
    plt.plot(steps, val_losses['mu_raw'], 'r-', label='Val (Raw)', linewidth=2)
    plt.xlabel('Step')
    plt.ylabel('MSE Loss')
...
def main():
    ...
    for step in range(1, config['num_steps'] + 1):
        ...
        if step % config['eval_interval'] == 0 or step == 1:
            ...
            train_losses['mu_raw'].append(train_components['loss_mu_raw'])
            ...
            val_losses['mu_raw'].append(val_components['loss_mu_raw'])
            ...
            logger.info(f"  Raw MSE Loss    - E: {train_components['loss_E_raw']:.6f}/{val_components['loss_E_raw']:.6f}, "
                       f"mu: {train_components['loss_mu_raw']:.6f}/{val_components['loss_mu_raw']:.6f}, "
