...
=========================================================
本脚本读取已处理好的 .pt 数据，使用代码中的物理公式重新计算 R (R_calc)，
...
def parse_args():
    parser = argparse.ArgumentParser(description='检查物理公式计算的R与标签R的匹配度')
...
def check_formula_consistency(data_path):
    ...
    print("\n【抽样详细对比 (前10个激发态)】")
    print(f"{'State':<10} | {'R_true (标签)':<15} | {'R_calc (公式计算)':<18} | {'绝对误差':<10}")
    print("-" * 60)
    for i in range(10):
    ...
    if correlation.item() > 0.99:
        print("-> 结论：皮尔逊相关系数极高，说明公式的**物理形式（线性关系）是完全正确的**。")
        print("-> 注意：如果存在小的绝对误差，极大概率是因为 Gaussian .log 文件输出的 μ_vel 和 m")
        print("   仅仅保留了 4 位小数（例如 0.0268），存在严重的**数值截断误差**。")
    else:
        print("-> 结论：相关系数不理想，**公式本身或单位换算常数存在严重问题**！")
        print("   需要重新推导 6414.135151 这个常数，或者检查是否有点积方向的反转。")
