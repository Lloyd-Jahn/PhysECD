...
def main():
    ...
    config = {
        ...
        # ==========================================
        # Loss Weights - 两阶段训练策略
        # ==========================================
        # 阶段1 (epoch 1-15): 只训练 E, μ, m，不监督 R
        # 阶段2 (epoch 16-25): 加入 R 的监督
        'lambda_E': 1.0,
        'lambda_mu': 10.0,
        'lambda_m': 50.0,
        'lambda_R': 0.0,  # 初始为0，后续动态调整
        'warmup_epochs': 15  # 前15个epoch不监督R
    }
    ...
    for epoch in range(1, config['num_epochs'] + 1):
        ...
        # 动态调整 lambda_R：前 warmup_epochs 个 epoch 不监督 R
        if epoch <= config['warmup_epochs']:
            criterion.update_weights(lambda_R=0.0)
            logger.info(f"  [Warmup Phase] lambda_R = 0.0 (只训练 E, μ, m)")
        elif epoch == config['warmup_epochs'] + 1:
            criterion.update_weights(lambda_R=0.1)
            logger.info(f"  [Fine-tune Phase] lambda_R = 0.1 (开始监督 R)")
