"""
读取 .pt 文件中第 x 个分子的所有数据
用法：python scripts/01_read_molecule_data.py --index x
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description='读取 .pt 文件中的指定分子数据')
    parser.add_argument('--file_path', type=str, default='data/processed_with_enantiomers/train.pt',
                        help='.pt 文件的路径')
...
def main():
    ...
    if not isinstance(data_list, list):
        raise TypeError("文件内容不是列表格式，可能不是有效的 .pt 文件")
