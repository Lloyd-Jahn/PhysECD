...
/home/jiangyi/.conda/envs/ecd_pred/bin/python /home/data/jiangyi/PhysECD/scripts/06_generate_pred_spectrum.py --mol_idx 5
最后的数字“5”可以自己指定
"""
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='Index of molecule in test set to predict (default: 0)'
    )
    parser.add_argument(
        '--output_dir',
        type=str,
...
def main():
    ...
    # Select molecule
    print(f"\n[2/4] Selecting molecule for inference...")
    if args.mol_idx >= len(test_data):
        raise ValueError(f"mol_idx {args.mol_idx} out of range (test set has {len(test_data)} molecules)")

    data = test_data[args.mol_idx]
    mol_id = data.mol_id
    print(f"  Selected molecule index: {args.mol_idx}")
